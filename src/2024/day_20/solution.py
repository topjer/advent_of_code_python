...
def find_shortcut(point, index, path):
    new_points = [(point[0] + diff[0], point[1] + diff[1]) for diff in DIFFS]
    shortcuts = [(path[point] - index - 2) for point in new_points if (point in path) and (path[point] > index + 2)]
    # shortcuts = [point for point in new_points if (point in path) and (path[point] > index + 2)]
    return shortcuts
...
def part_01(task_input) -> int:
    ...
    for point, index in path.items():
        shortcuts = find_shortcut(point, index, path)
